---
region: lowback
id: stenosis
name: Spinal stenosis pattern
clin: Lumbar spinal stenosis / neurogenic claudication
# Shown only from age 50 (as before).
ages: 50-64, o64
# Patient text moved unchanged from the site's original low back region.
# Pointers from content/regions/lowback.md, 24 Sep 2026.
# Reviewed by: (add your name once you have checked every line)
pointers:
  "Gradually, no clear reason": 1
  "More than 3 months": 1
  "Below the knee, into the leg or foot": 1
  "Down the thigh, stopping above the knee": 1
  "About the same": 1
  "The leg pain is worse when I bend forward or sit": -2
  "Arching back, or standing for a long time": 2
  "Walking brings on leg pain, heaviness, or tingling that eases when I sit or bend forward": 3
---

## blurb
With age, the passageways for the spinal nerves can narrow. The classic pattern: legs that ache, heavy or tingle with walking, easing quickly when you sit or lean forward (like on a shopping cart).

## noticed
- Walking distance limited by leg heaviness or ache
- Relief within minutes of sitting or bending forward
- Often better cycling than walking

## homeCare
- Keep walking within your comfortable distance — little and often
- Cycling or pool walking maintain fitness with less symptom
- A slight forward lean (poles, cart) extends walking range

## seePhysioIf
- Walking distance is shrinking
- You'd like a structured conditioning plan — good evidence supports exercise for this pattern
- Symptoms affect balance or confidence
