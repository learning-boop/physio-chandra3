/* Vercel serverless function → served at /api/pain-analysis.
   ...
   Chandra's approved content. The conditions the app's scoring matched are
   passed too, and the overview explains exactly those — so it can never
   contradict the result cards above it. If nothing matched, the prompt says
   so and routes to an assessment instead of inventing specifics; any
   malformed output falls back to fixed safe content. */
...
export default async function handler(req, res) {
  ...
  if (!hasValidKey(API_KEY)) {
    return res.status(200).json(fallbackAnalysis(labels, found))
  }
  ...
  try {
    ...
    const kept = applyReview(found, review)
    return res.status(200).json({ ...sanitizeAnalysis(parsed, labels, kept), review })
  } catch (err) {
